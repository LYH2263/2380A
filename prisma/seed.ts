...
async function main() {
  ...
  await prisma.eventDailyStat.deleteMany()
  await prisma.eventParticipation.deleteMany()
  await prisma.event.deleteMany()
  await prisma.announcementDismiss.deleteMany()
...
这两个家伙，真的是让人又头疼又感到骄傲啊。`
  ...
  // ==================== 将小说/章节/评论设为已审核通过（否则前台不显示）====================
  await prisma.novel.updateMany({
    data: { reviewStatus: 'APPROVED' }
  })
  await prisma.chapter.updateMany({
    data: { reviewStatus: 'APPROVED', status: 'PUBLISHED', publishedAt: new Date() }
  })
  ...
  // ==================== 评论点赞 ====================
  const allComments = await prisma.comment.findMany({ take: 4 })
  if (allComments.length > 0) {
    await prisma.commentLike.createMany({
      data: [
        { userId: admin.id, commentId: allComments[0].id },
        { userId: user2.id, commentId: allComments[0].id },
        { userId: user1.id, commentId: allComments[1]?.id ?? allComments[0].id }
      ],
      skipDuplicates: true
    })
  }
  ...
  // ==================== 阅读历史 ====================
  const novel1FirstChapter = await prisma.chapter.findFirst({ where: { novelId: novel1.id }, orderBy: { order: 'asc' } })
  const novel3FirstChapter = await prisma.chapter.findFirst({ where: { novelId: novel3.id }, orderBy: { order: 'asc' } })
  await prisma.readHistory.createMany({
    data: [
      { userId: user1.id, novelId: novel1.id, lastChapterId: novel1FirstChapter?.id, progress: 65 },
      { userId: user1.id, novelId: novel3.id, lastChapterId: novel3FirstChapter?.id, progress: 100 },
      { userId: user2.id, novelId: novel1.id, lastChapterId: novel1FirstChapter?.id, progress: 30 }
    ],
    skipDuplicates: true
  })
  ...
  const bookList1 = await prisma.bookList.create({
    data: {
      userId: user1.id,
      name: '我的最爱',
      description: '收藏的最喜欢的 Neuro 同人作品',
      color: '#a855f7',
      sortOrder: 0,
      isDefault: true
    }
  ...
  // 把 user1 的收藏加入书单
  const user1Favorites = await prisma.favorite.findMany({ where: { userId: user1.id } })
  if (user1Favorites.length > 0) {
    await prisma.bookListItem.createMany({
      data: user1Favorites.map(fav => ({ bookListId: bookList1.id, favoriteId: fav.id })),
      skipDuplicates: true
    })
  }
  ...
  await prisma.announcement.createMany({
    data: [
      {
        title: '🎉 欢迎来到 Neurosama 粉丝小说站！',
        content: '本站是 Neuro-sama 粉丝二创小说的聚集地，欢迎大家投稿原创作品、阅读和评论。请遵守社区规范，共同营造友好的创作氛围。',
        priority: 'HIGH',
        target: 'ALL',
        startAt: weekAgo,
        endAt: monthLater,
      ...
      {
        title: '📢 第一届同人征文大赛开始啦',
        content: '以"Neuro 与 Vedal 的日常"为主题进行创作，优秀作品将获得首页推荐和精美周边！活动详情请查看活动页面。',
        priority: 'URGENT',
        target: 'ALL',
        startAt: weekAgo,
        endAt: weekLater,
      ...
      {
        title: '🔧 系统维护通知',
        content: '为提升服务质量，本站将不定期进行系统维护，维护期间可能出现短暂访问异常，敬请谅解。',
        priority: 'MEDIUM',
        target: 'ALL',
        startAt: weekAgo,
        endAt: monthLater,
  ...
  const event1 = await prisma.event.create({
    data: {
      title: '第一届 Neuro 同人征文大赛',
      description: '以 Neuro-sama 为主题的原创同人小说征文活动，让我们一起用文字记录对 Neuro 的喜爱！',
      rules: '1. 作品须为原创且与 Neuro-sama 相关\n2. 字数不少于 2000 字\n3. 内容健康向上\n4. 每位作者最多提交 3 部作品\n5. 由读者投票 + 评委评分决定最终名次',
      coverImage: 'https://images.unsplash.com/photo-1513475382585-d06e58bcb0e0?w=800',
      type: 'CONTEST',
      status: 'ONGOING',
      registrationStartAt: weekAgo,
      registrationEndAt: weekLater,
      eventStartAt: weekAgo,
      eventEndAt: monthLater,
      minWordCount: 2000,
      allowedTags: ['Neuro-sama', '日常', '科幻', '喜剧'],
      requireNew: false,
      sortOrder: 0,
      createdById: admin.id
    }
  ...
  await prisma.eventParticipation.createMany({
    data: [
      { eventId: event1.id, novelId: novel1.id, userId: admin.id, status: 'APPROVED', voteCount: 42, approvedAt: now },
      { eventId: event1.id, novelId: novel2.id, userId: user1.id, status: 'APPROVED', voteCount: 28, approvedAt: now },
      { eventId: event1.id, novelId: novel3.id, userId: user2.id, status: 'APPROVED', voteCount: 56, approvedAt: now }
    ],
    skipDuplicates: true
  })
  ...
  await prisma.sensitiveWord.createMany({
    data: [
      { word: '示例敏感词', level: 'WARN', category: '测试' },
      { word: '违规内容', level: 'BLOCK', category: '测试' },
      { word: '广告推广', level: 'WARN', category: '垃圾信息' }
    ],
    skipDuplicates: true
  })
  ...
  console.log('✅ 数据填充完成！')
  console.log('----------------------------------------')
  console.log('📚 创建了 4 本小说（已审核通过）')
  console.log('👤 创建了 3 个用户')
  console.log('📖 创建了多个章节')
  console.log('💬 创建了多条评论')
  console.log('📢 创建了 3 条公告 / 3 个 Banner / 2 个活动')
  console.log('🔖 创建了书单、关注、阅读历史、敏感词等数据')
  console.log('----------------------------------------')
  console.log('🔑 测试账号:')
