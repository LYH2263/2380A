  ...
  <div class="min-h-screen py-8">
    <div class="container mx-auto px-4">
      <div class="flex items-center gap-4 mb-8">
        ...
        <h1 class="text-2xl font-bold">{{ username }} 的粉丝</h1>
      </div>
      ...
      <div v-else-if="followers.length === 0" class="text-center py-20">
        <Icon name="ph:users-three" class="text-6xl text-white/30 mb-4" />
        <p class="text-xl text-white/50">还没有任何粉丝</p>
...
const { data, pending, refresh } = await useFetch(
  () => `/api/user/${username.value}/followers?page=${page.value}`,
  { watch: [username, page] }
)
...
useHead(() => ({
  title: `${username.value} 的粉丝 - Neurosama 粉丝小说站`
}))
</script>
